...
def run_explainability_analysis(model_dir):
    ...
    generate_hierarchical_diagnostics(attention_weights, results, model_dir)
...
def generate_hierarchical_diagnostics(attention_weights, results, model_dir):
    """
    ...
    for sample_idx, sample_layers in enumerate(attention_weights):
        ...
        if sample_idx < len(raw_sg_data):
            ...
            if 'our_metrics' in results and 'y_prob' in results['our_metrics'] and sample_idx < len(results['our_metrics']['y_prob']):
                predicted_label = int(results['our_metrics']['y_prob'][sample_idx] >= 0.5)  # threshold at 0.5
