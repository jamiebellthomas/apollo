#!/usr/bin/env python3
"""
reproduce_run_heterognn5.py

Reloads a finished HeteroGNN5 run:
- Loads cached training/testing graphs
- Parses hyperparameters.txt
- Rebuilds HeteroGNN5 with those params
- Loads weights from model.pt
- Re-splits train/val using saved ratios/seed
- Finds validation threshold
- Evaluates on test set

Modified to work with our cached data structure.
"""
...
def find_optimal_threshold(model, val_loader, device):
    ...
    with torch.no_grad():
        for batch in val_loader:
            batch = batch.to(device)
            all_probs.append(torch.sigmoid(model(batch)).cpu())
...
def main():
    print("=" * 60)
    print("REPRODUCING EXACT EVALUATION WITH GPT-5 CODE")
    print("=" * 60)
    ...
    test_loader  = DataLoader(test_graphs, batch_size=2 * batch_size, shuffle=False)  # Original uses 2 * batch_size
    ...
    metrics = evaluate(model, test_loader, device, threshold=th)
